...
function mergeDashboardData(current, incoming) {
    ...
    if (incoming.trades) {
        merged.trades = { ...(current.trades || {}), ...incoming.trades };
        const currentTrades = current.trades || {};
        const incomingTrades = incoming.trades || {};
        const currentCount = (currentTrades.closed || []).length + (currentTrades.signals || []).length;
        const incomingCount = (incomingTrades.closed || []).length + (incomingTrades.signals || []).length;
        const incomingTotal = Number(incomingTrades.closed_total || 0) + Number(incomingTrades.signals_total || 0);
        if (currentTrades.full_history_loaded && incomingTrades.window && currentCount >= incomingTotal && incomingCount < currentCount) {
            merged.trades.closed = currentTrades.closed || [];
            merged.trades.signals = currentTrades.signals || [];
            merged.trades.full_history_loaded = true;
        }
    }
...
function tradeHistoryTotals(trades) {
    ...
    const visible = (trades.closed || []).length + (trades.signals || []).length;
    return { closedTotal, signalsTotal, total: closedTotal + signalsTotal, visible };
}

async function maybeHydrateFullTradeHistory(data, trades) {
    if (!data || !trades || trades.full_history_loaded) return;
    const totals = tradeHistoryTotals(trades);
    if (totals.total <= totals.visible) return;

    const mode = String(data.mode || data.config?.mode || '').toUpperCase();
    const source = String(trades.meta?.source || '').toLowerCase();
    const isBacktest = mode === 'HISTORICAL' || source.includes('historical') || source.includes('simulation');
    if (!isBacktest) return;

    const simKey = `${data.simulation_id || 0}:${totals.total}:${trades.meta?.source || ''}`;
    if (state.tradeHistoryHydration.inFlight || state.tradeHistoryHydration.key === simKey) return;
    state.tradeHistoryHydration.inFlight = true;
    state.tradeHistoryHydration.key = simKey;

    try {
        const limit = Math.max(250, Math.min(1000, totals.total));
        const mergedPayload = { ...trades, closed: [], signals: [] };
        let offset = 0;
        while (offset < totals.total) {
            const response = await fetch(`/api/trades?limit=${limit}&offset=${offset}`, { cache: 'no-store' });
            if (!response.ok) throw new Error(`Trade history fetch failed: ${response.status}`);
            const page = await response.json();
            mergedPayload.closed.push(...(page.closed || []));
            mergedPayload.signals.push(...(page.signals || []));
            mergedPayload.open = page.open || mergedPayload.open || [];
            mergedPayload.summary = page.summary || mergedPayload.summary || {};
            mergedPayload.analytics = page.analytics || mergedPayload.analytics || {};
            mergedPayload.meta = page.meta || mergedPayload.meta || {};
            mergedPayload.closed_total = page.closed_total ?? totals.closedTotal;
            mergedPayload.signals_total = page.signals_total ?? totals.signalsTotal;

            const pageCount = (page.closed || []).length + (page.signals || []).length;
            if (pageCount === 0) break;
            offset += limit;
            const latestTotal = Number(mergedPayload.closed_total || 0) + Number(mergedPayload.signals_total || 0);
            if (mergedPayload.closed.length + mergedPayload.signals.length >= latestTotal) break;
        }

        mergedPayload.full_history_loaded = true;
        state.latestData = mergeDashboardData(state.latestData, { trades: mergedPayload, simulation_id: data.simulation_id });
        updateTradesPanel(state.latestData.trades || {}, data.simulation_id);
        updatePnlPanel(state.latestData.trades || {});
        updateEquityReplacement(state.latestData.trades || {});
    } catch (error) {
        console.warn('Full trade history hydration failed', error);
        state.tradeHistoryHydration.key = '';
    } finally {
        state.tradeHistoryHydration.inFlight = false;
    }
}
...
function updateDashboard(data) {
    ...
    maybeHydrateFullTradeHistory(data, tradePayload);
...
function updateTradesPanel(trades, simId) {
    ...
    if (modeBadge) {
        ...
        const totals = tradeHistoryTotals(trades);
        const expectedRows = restored + simulated || totals.total;
        const visibleLabel = totals.total > totals.visible
            ? ` showing ${totals.visible}/${totals.total}`
            : '';
    ...
    const summary = buildSessionSummary(trades.summary || {}, open, closed, signalRows);
