...
except ModuleNotFoundError:  # pragma: no cover - depends on local Windows desktop package state
    Toast = InteractableWindowsToaster = ToastButton = ToastAudio = AudioSource = None

logger = logging.getLogger(__name__)

class NotificationManager:
    def __init__(self, loop: Optional[asyncio.AbstractEventLoop] = None):
        self.toaster = InteractableWindowsToaster('UT Bot Pro') if InteractableWindowsToaster else None
        self.loop = loop
    ...
    def send_trade_notification(
        ...
        if self.toaster is None or Toast is None:
            logger.warning("Desktop toast notification skipped: windows_toasts is not installed.")
            return

        toast = Toast()
        
        # Audio cues
        if ToastAudio is not None and AudioSource is not None:
